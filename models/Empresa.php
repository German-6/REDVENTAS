<?php
    class Empresa extends Conectar{
        ...
        /* TODO: Registro Datos */
        public function insert_empresa($com_id,$emp_nom,$emp_rut){
            $conectar=parent::Conexion();
            $sql="SP_I_EMPRESA_01 ?,?,?";
            $query=$conectar->prepare($sql);
            $query->bindValue(1,$com_id);
            $query->bindValue(2,$emp_nom);
            $query->bindValue(3,$emp_rut);
            $query->execute();
        }

        /* TODO: Actualizar Datos */
        public function update_empresa($emp_id,$com_id,$emp_nom,$emp_rut){
            $conectar=parent::Conexion();
            $sql="SP_U_EMPRESA_01 ?,?,?,?";
            $query=$conectar->prepare($sql);
            $query->bindValue(1,$emp_id);
